...
<div class="gl-archive">
    ...
    foreach ( $archive_zones as $zone_id => $zone_cfg ) :
        ...
        switch ( $zone_id ) :
            ...
            case 'pinned_hubs':
                // Show pinned hub pages for the current category
                if ( is_category() ) :
                ...
                <section class="gl-zone gl-zone--pinned-hubs" data-zone="pinned_hubs">
                    <div class="gl-container">
                        ...
                        <div class="gl-card-grid gl-card-grid--3col">
                            ...
                            ?>
                            <a href="<?php the_permalink(); ?>" class="gl-hub-card" style="--cat-accent: <?php echo esc_attr( $cat_color ); ?>">
            ...
            case 'filter_bar':
                // Post type filter for category archives
                if ( is_category() ) :
                    $cat_obj = get_queried_object();
                ?>
                <div class="gl-zone gl-zone--filter-bar" data-zone="filter_bar">
                    <div class="gl-container">
                        <div class="gl-archive-filters">
                            <a href="<?php echo esc_url( get_category_link( $cat_obj->term_id ) ); ?>" class="gl-hub-filter gl-hub-filter--active"><?php esc_html_e( 'All', 'gtalobby' ); ?></a>
                            <?php foreach ( gtalobby_get_post_types() as $pt_slug ) :
                                $pt_obj = get_post_type_object( $pt_slug );
                                if ( ! $pt_obj ) continue;
                            ?>
                            <a href="<?php echo esc_url( add_query_arg( 'post_type', $pt_slug, get_category_link( $cat_obj->term_id ) ) ); ?>" class="gl-hub-filter">
            ...
            case 'pagination':
                // Handled inside post_grid zone above
                break;
